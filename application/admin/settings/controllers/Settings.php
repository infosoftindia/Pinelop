...
class Settings extends MX_Controller
{
	...
	public function save_Site_Legal()
	{
		allowUser([121]);
		$data = [
			'pages_description' => $this->input->post('terms'),
		];
		$this->db->where('pages_id', '1')->update('pages', $data);
		$data = [
			'pages_description' => $this->input->post('policies'),
		];
		$this->db->where('pages_id', '2')->update('pages', $data);
		$data = [
