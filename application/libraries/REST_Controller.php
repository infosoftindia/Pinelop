...
abstract class REST_Controller extends \CI_Controller {
     ...
     * @return void
     */
    protected function auth_User($api)
    {
        $this->db->where('api_token', $api);
        $data = $this->db->get('api');
		if(!$data->num_rows()){
			echo $this->response([
				$this->config->item('rest_status_field_name') => FALSE,
				$this->config->item('rest_message_field_name') => sprintf($this->lang->line('text_rest_invalid_api_key'), $api)
			], self::HTTP_UNAUTHORIZED);
			// die;
			return 0;
		}else{
